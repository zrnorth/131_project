...
# METTA uses port 12974
# METTA talks to BRYANT(12971)
...
import sys
log.startLogging(open('metta.log','w'))

import time


class Metta(LineReceiver):
	...
	def connectionMade(self):
		self.sendLine("You have connected to Metta server. (debug)")
	def connectionLost(self, reason):
		return
...
class MettaFactory(Factory):
	def __init__(self):
		self.user_info = {}
	def buildProtocol(self, addr):
		return Metta(self.user_info)

factory = MettaFactory()
reactor.listenTCP(12974, factory)
reactor.run()
