# GASOL server implementation
# GASOL uses port 12972
# GASOL talks to BRYANT(12971) and HOWARD(12973)
...
import sys
log.startLogging(open('gasol.log','w'))

import time


class Gasol(LineReceiver):
	...
	def connectionMade(self):
		self.sendLine("You have connected to Gasol server. (debug)")
	def connectionLost(self, reason):
		return
...
class GasolFactory(Factory):
	def __init__(self):
		self.user_info = {}
	def buildProtocol(self, addr):
		return Gasol(self.user_info)

factory = GasolFactory()
reactor.listenTCP(12972, factory)
reactor.run()
