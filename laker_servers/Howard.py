# HOWARD server implementation
# HOWARD uses port 12973
# HOWARD talks to BLAKE(12970) and GASOL(12972)
...
import sys
log.startLogging(open('howard.log','w'))

import time


class Howard(LineReceiver):
	...
	def connectionMade(self):
		self.sendLine("You have connected to Howard server. (debug)")
	def connectionLost(self, reason):
		return
...
class HowardFactory(Factory):
	def __init__(self):
		self.user_info = {}
	def buildProtocol(self, addr):
		return Howard(self.user_info)

factory = HowardFactory()
reactor.listenTCP(12973, factory)
reactor.run()
